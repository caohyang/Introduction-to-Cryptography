...
int mul(int x,int y) {return table[(arc_table[x]+arc_table[y])%255];}
int num(char x){
...
int main(){		
	...
	//BM Algorithm 
	int v[20000],len[4]={1e3,5e3,1e4,2e4}; 
	int C[10005],B[10005],T[10005],L,m,d,N;
		
	for (int round=0;round<4;round++){
		printf("The length is %d.\n",len[round]);
		for (int subround=0;subround<=10;subround++){
			printf("Round %d: ",subround);
			srand((int)time(0));
			loc=rand()%(cl-len[round]/8);
			for (i=loc;i<loc+len[round]/8;i++){
				k=cipher[i];
				for (j=7;j>=0;j--){
					v[(i-loc)*8+j]=k%2;
					k/=2;
			...
			else printf("The linear complexity is %d.\n",L);
